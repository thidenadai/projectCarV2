  ...
  console.log(
    `${
      req.session.username
        ? `Usuário ${req.session.username} logado no IP ${req.connection.remoteAddress}`
        : "Usuário não logado."
    }  `
  );
...
app.get("/cadastro", (req, res) => {
  if (!req.session.loggedin) {
    res.render("pages/cadastro", { req: req });
  } else {
...
app.post("/cadastrar", (req, res) => {
  ...
  db.query(query, [username, password], (err, results) => {
    ...
    if (results.length > 0) {
      console.log(
        `Usuário ${username} já existe no banco de dados. redirecionando`
      );
